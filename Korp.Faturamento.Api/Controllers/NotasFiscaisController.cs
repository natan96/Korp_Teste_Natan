...
namespace Korp.Faturamento.Api.Controllers
{
    ...
    public class NotasFiscaisController : ControllerBase
    {
        ...
        /// <summary>
        /// Obtém todas as notas fiscais de forma paginada
        /// </summary>
        [HttpGet]
        [ProducesResponseType(typeof(NotaFiscalPagedResult), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<NotaFiscalPagedResult>> GetAll(
            ...
            [FromQuery] int? status = null)
        {
            try
            {
                var (items, totalCount, totalAbertas, totalFechadas) =
                    await _notaFiscalService.GetPagedAsync(page, pageSize, status);

                return Ok(new NotaFiscalPagedResult
                {
                    Items = items.Select(nf => MapToDto(nf)).ToList(),
