...
namespace Korp.Estoque.Api.Controllers
{
    ...
    public class ProdutosController : ControllerBase
    {
        ...
        /// </summary>
        [HttpGet]
        [ProducesResponseType(typeof(PagedResult<Produto>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<PagedResult<Produto>>> GetAll(
            ...
            [FromQuery] string? search = null)
        {
            try
            {
                var result = await _produtoService.GetPagedAsync(page, pageSize, search);
                return Ok(result);
            }
            catch (Exception ex)
            {
