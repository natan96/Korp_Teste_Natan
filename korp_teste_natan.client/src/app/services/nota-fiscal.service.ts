...
export class NotaFiscalService {
  ...
  getPaged(
    ...
    pageSize: number,
    status?: number,
  ): Observable<NotaFiscalPagedResult> {
    ...
    return this.http
      .get<NotaFiscalPagedResult>(this.apiUrl, { params })
      .pipe(retry(2), catchError(this.handleError));
  }

  getById(id: number): Observable<NotaFiscal> {
    return this.http
      .get<NotaFiscal>(`${this.apiUrl}/${id}`)
      ...
      retry({
        ...
        delay: (error, retryCount) => {
          ...
          if (error.status === 503) {
            ...
            console.log(
              `Tentativa ${retryCount} falhou. Tentando novamente em ${delayMs}ms...`,
            );
  ...
  checkEstoqueStatus(): Observable<any> {
    return this.http
      .get(`${this.apiUrl}/estoque/status`)
      .pipe(
        catchError(() =>
          throwError(() => new Error('Não foi possível verificar status')),
        ),
      );
  }
